...
frames_dir = os.path.join(ros2_ws_path, 'map_data/ESL_FINAL_BAG/frames')
lidar_dir = os.path.join(ros2_ws_path, 'map_data/ESL_FINAL_BAG/lidar_scans')
pose_dir = os.path.join(ros2_ws_path, 'map_data/ESL_FINAL_BAG/poses')
...
def main(args=None):
    ...
    node = Node('camera_lidar_saver_voyager')

    clear_folder(frames_dir)
    clear_folder(lidar_dir)
    clear_folder(pose_dir)

    node.create_subscription(Image, '/webcam/image_raw', image_callback, 10)
